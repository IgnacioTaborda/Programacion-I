    ...
    def btn_cargar_ventas_on_click(self):
        ...
        while cantidad_compradores < limite_entradas_a_vender:
            ...
            while nombre == None or not nombre.isalpha():
                nombre = input(f"Ingrese el nombre del cliente {cantidad_compradores+ 1}°: ")
                
            #Edad (no menor a 18)
            edad = 0
            while edad == 0 or not edad.isdigit() or (int(edad) < 18):
                edad = input(f"Ingrese la edad del cliente {cantidad_compradores+ 1}°: ")
            edad_int = int(edad)
            
            #Género (Masculino, Femenino, Otro)
            genero = None
            while genero == None or not genero.isalpha() or (genero != "Masculino" and genero != "Femenino" and genero != "Otro"):
                genero = input(f"Ingrese el genero del cliente {cantidad_compradores+ 1}°: ").capitalize()
                
            #Tipo de entrada (General, Campo delantero, Vip)   
            tipo = None
            while tipo == None or not tipo.isalpha() or (tipo != "General" and tipo != "Campo delantero" and tipo != "Vip"):
                tipo = input(f"Ingrese el tipo de entrada del cliente {cantidad_compradores+ 1}°: ").capitalize()   
                
            #Medio de pago (Crédito, Efectivo, Débito) 
            medio = None
            while medio == None or not medio.isalpha() or (medio != "Crédito" and medio != "Efectivo" and medio != "Débito"):
                medio = input(f"Ingrese el medio de pago elegido del cliente {cantidad_compradores+ 1}°: ").capitalize()   
        ...
        #7) - Nombre de la/s persona/s de menor edad, de género Masculino que compro una entrada general.      
